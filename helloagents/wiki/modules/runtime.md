...
- `styles.css`
  ...
  - EVO-VIS v4（Pixel UI）：更克制的中性色与“hairline border + 精准阴影”层级；卡片追光（`--fx-x/--fx-y`）、磁吸位移（`--fx-tx/--fx-ty`）、按压态（`.is-pressed`）与点击 Ripple（`.fx-ripple`）统一微交互手感（尊重 Reduced Motion/Transparency）
