...
- [√] 2.2 在 `src/runtime/pages/` 建立页面入口（按 `data-page` 精确 init）
  - 依赖: 2.1
  - 交付: `src/runtime/pages/registry.mjs` + `tests/runtime-pages.test.mjs`
  - 验证: 任意页面不会初始化不相关逻辑（可通过日志/断言验证）

- [√] 2.3 将现有 `scripts.js` 逐步迁移到模块化入口（保留 IIFE 入口作为兼容层）
  - 依赖: 2.2
  - 说明: v1 阶段以“先 core 可测 + pages 入口结构化”为主；`scripts.js` 继续作为 classic IIFE 兼容层，后续迭代按模块逐步替换内部实现
...
- [√] 3.2 形成“发布命令”：串联 build + 校验 + bump version + 生成 feed/sitemap
  - 交付: `tools/release.mjs` + npm scripts `release` / `release:check`
...
- [√] 4.3 信息架构收敛：标签/分类规范化（统一大小写、同义合并、展示规则）
  - 交付: `content/taxonomy.json` + `tools/{build-data,validate-data}.mjs` taxonomy 约束
  - 验证: 数据校验可阻止新增“脏标签”
...
- [√] 6.1 为关键页面补齐结构化数据（JSON-LD）与 Open Graph（可批量生成）
  - 交付: `tools/inject-seo.mjs` 批量补齐 + `tools/check-html.mjs` 门禁约束
