...
- FLIP Layout（All Games）：网格/列表切换、排序、筛选显隐都使用 FLIP（First-Last-Invert-Play）做布局动画；筛选“退出”的卡片会短暂进入绝对定位 overlay（`.game-card.is-filter-exiting`）执行 exit 动画，结束后再设置 hidden；只动画 transform/opacity/filter，并尊重 `prefers-reduced-motion`（保持 60FPS 合成优先）
